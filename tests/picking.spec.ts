...
test('Staff should see new task with correct info', async ({ page, configuration, pickingService, pickingPage }) => {
  // Arrange
  pickingRequestKey = null;
...
test('Staff should see task detail with correct item information', async ({ page, configuration, pickingService, pickingPage, taskDetailPage }) => {
  pickingRequestKey = null;
  pickingRequestKey = pickingService.generatePickingRequestKey();
  items = [configuration.products.itemA];
  await pickingService.createPickingRequest(pickingRequestKey, items);
  await pickingPage.switchToAllTasks();
  const task = await pickingPage.getTaskByOrderNo(pickingRequestKey.sourceOrderId);
  const taskInfo = await task.getTaskInfo();
  ...
  expect(await taskDetailPage.getTaskId())
    .toEqual(taskInfo.taskId);
  expect(await taskDetailPage.getPickRequestId())
    .toEqual(pickingRequestKey.pickRequestId);
  expect(await taskDetailPage.getTotalItemCount())
    .toEqual(1);
});
